...
public class ServicioWebCarrito {
	...
	public ResponseEntity<String> agregarVideojuego(String id, String cantidad, HttpServletRequest request) {
		String respuesta = "";
		if(request.getSession().getAttribute("usuario") != null) {
			//esto es que el usuario se identificó
...
//					+ " del usuario: " + u.getNombre();
	...
	public ResponseEntity<String> obtenerProductosCarrito(HttpServletRequest request) {
		String respuesta = "";
		respuesta = new Gson().toJson(carrito.obtenerProductosCarrito((Usuario) request.getSession().getAttribute("usuario")));
		return new ResponseEntity<String>(respuesta, HttpStatus.OK);
	}
	...
	public ResponseEntity<String> borrarProductoCarrito(String idProducto, HttpServletRequest request) {
		String respuesta = "";
		int idProducto_int = Integer.parseInt(idProducto);
		int idUsuario = ((Usuario) request.getSession().getAttribute("usuario")).getId();
		carrito.borrarProductoCarrito(idUsuario, idProducto_int);
		respuesta = "ok";
		return new ResponseEntity<String>(respuesta, HttpStatus.OK);
	}
